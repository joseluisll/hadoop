...
# Set from the command line. Upstream derives every path from the script's own
# location inside the Hadoop tree, which does not work for a copy kept outside it.
REPO_DIR_OVERRIDE = None
WORK_DIR_OVERRIDE = None
JAVA_ACC_DIR_OVERRIDE = None
...
def get_java_acc_dir():
  ...
  if JAVA_ACC_DIR_OVERRIDE:
    return JAVA_ACC_DIR_OVERRIDE
...
def checkout_java_acc(force):
  ...
  """
  if JAVA_ACC_DIR_OVERRIDE:
    checker = os.path.join(JAVA_ACC_DIR_OVERRIDE,
                           "japi-compliance-checker.pl")
    if not os.path.exists(checker):
      logging.error("No japi-compliance-checker.pl under %s",
                    JAVA_ACC_DIR_OVERRIDE)
      sys.exit(1)
    logging.info("Using the Java ACC supplied at %s", JAVA_ACC_DIR_OVERRIDE)
    return
  ...
  try:
    ...
    logging.error("Could not download the Java ACC from %s: %s", url, exc)
    logging.error("Restricted networks commonly block this. Clone it instead:")
    logging.error("  git clone -b 1.8 "
                  "https://github.com/lvc/japi-compliance-checker <dir>")
    logging.error("then re-run with --java-acc-dir <dir>")
    sys.exit(1)
...
def run_java_acc(src_name, src_jars, dst_name, dst_jars, annotations,
  ...
  # Without this the checker silently drops every package named "impl" or
  # "internal", which is a large part of the surface the ecosystem actually uses.
  if keep_internal:
    args += ["-keep-internal"]

  # The checker exits 1 when it finds an incompatibility, which is a result and
  # not a failure. Only an unexpected code means the run itself went wrong.
  rc = subprocess.call(args)
  if rc == 0:
    logging.info("No incompatibilities found. Report: %s", out_path)
  elif rc == 1:
    logging.warning("Incompatibilities found. Report: %s", out_path)
  else:
    logging.error("The compliance checker failed with exit code %d", rc)
    sys.exit(1)
  return rc
...
def main():
  ...
  parser.add_argument("--repo-dir",
                      type=str,
                      help="Path to the Hadoop git repository to operate on. "
                      "Required when this script lives outside that tree.")
  parser.add_argument("--work-dir",
                      type=str,
                      help="Scratch directory to build within. "
                      "Defaults to <repo>/target/compat-check.")
  parser.add_argument("--java-acc-dir",
                      type=str,
                      help="Path to an existing Java API Compliance Checker "
                      "checkout. Skips the download, which restricted networks "
                      "commonly block.")
  parser.add_argument("--src-dir",
                      type=str,
                      help="Path to an already-built source tree. "
                      "It is neither checked out nor built.")
  parser.add_argument("--dst-dir",
                      type=str,
                      help="Path to an already-built destination tree. "
                      "It is neither checked out nor built.")
  parser.add_argument("--keep-internal",
                      action="store_true",
                      help="Also check packages named 'impl' or 'internal', "
                      "which the checker drops by default.")
  ...
  global REPO_DIR_OVERRIDE, WORK_DIR_OVERRIDE, JAVA_ACC_DIR_OVERRIDE
  if args.repo_dir:
    REPO_DIR_OVERRIDE = os.path.abspath(args.repo_dir)
  if args.work_dir:
    WORK_DIR_OVERRIDE = os.path.abspath(args.work_dir)
  if args.java_acc_dir:
    JAVA_ACC_DIR_OVERRIDE = os.path.abspath(args.java_acc_dir)
  ...
  # Set up the build.
  scratch_dir = get_scratch_dir()
  src_dir = os.path.abspath(args.src_dir) if args.src_dir \
      else os.path.join(scratch_dir, "src")
  dst_dir = os.path.abspath(args.dst_dir) if args.dst_dir \
      else os.path.join(scratch_dir, "dst")

  if args.skip_clean:
    logging.info("Skipping cleaning the scratch directory")
  else:
    clean_scratch_dir(scratch_dir)
    # Check out the src and dst source trees, unless they were supplied.
    if args.src_dir:
      logging.info("Using the source tree supplied at %s", src_dir)
    else:
      checkout_java_tree(get_git_hash(src_rev), src_dir)
    if args.dst_dir:
      logging.info("Using the destination tree supplied at %s", dst_dir)
    else:
      checkout_java_tree(get_git_hash(dst_rev), dst_dir)

  # Run the build in each. A supplied tree is already built; never rebuild
  # something the caller handed us.
  if args.skip_build:
    logging.info("Skipping the build")
  else:
    if args.src_dir:
      logging.info("Not building the supplied source tree")
    else:
      build_tree(src_dir)
    if args.dst_dir:
      logging.info("Not building the supplied destination tree")
    else:
      build_tree(dst_dir)
  ...
  rc = run_java_acc(src_rev, src_jars,
                    dst_rev, dst_jars, annotations, args.keep_internal)

  # 0: compatible. 2: incompatibilities found, which the caller reports as a
  # finding rather than treating as a broken run.
